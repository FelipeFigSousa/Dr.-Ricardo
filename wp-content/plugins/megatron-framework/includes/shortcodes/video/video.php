...
if (!class_exists('g5plusFramework_ShortCode_Video')) {
    class g5plusFramework_ShortCode_Video
    {
        ...
        {
            add_shortcode('megatron_video', array($this, 'video_shortcode'));
        }
        ...
        function video_shortcode($atts)
        {
             ...
             * @var $delay
	         */
            $image=$link=$text=$height=$shadow=$el_class='';
	        $atts = vc_map_get_attributes( 'megatron_video', $atts );
	        extract( $atts );
            ...
            wp_enqueue_style('megatron_video_css', plugins_url('megatron-framework/includes/shortcodes/video/assets/css/video' . $min_suffix_css . '.css'), array(), false);
            $style=' ';
            if (!empty($image)) {
            ...
            if($shadow=='yes')
            {
                $class = ' video-shadow ' . esc_attr($el_class) . g5plusFramework_Shortcodes::g5plus_get_css_animation($css_animation);
            }
            else
            {
            ...
            <div class="video<?php echo esc_attr($class) ?>" <?php echo g5plusFramework_Shortcodes::g5plus_get_style_animation($duration, $delay); ?>>
                <div class="video-bg"<?php echo wp_kses_post($style) ?>>
                    <a class="prettyPhoto s-font" data-rel="prettyPhoto" href="<?php echo esc_url($link) ?>">
                        <?php if($text!=''):?>
                        <i class="micon icon-right127"></i>
