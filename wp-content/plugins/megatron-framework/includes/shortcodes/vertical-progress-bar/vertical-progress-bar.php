...
if (!class_exists('g5plusFramework_ShortCode_Vertical_Progress_Bar')) {
    class g5plusFramework_ShortCode_Vertical_Progress_Bar
    {
        function __construct(){
            add_shortcode('megatron_vertical_progress_bar', array($this, 'vertical_progress_bar_shortcode'));
        }
        function vertical_progress_bar_shortcode($atts){
	        ...
	        $atts = vc_map_get_attributes( 'megatron_vertical_progress_bar', $atts );
	        extract( $atts );
            ...
            wp_enqueue_script('megatron_vertical_progress_bar_js', plugins_url('megatron-framework/includes/shortcodes/vertical-progress-bar/assets/js/vertical-progress-bar' . $min_suffix_js . '.js'), array('waypoints'), false, true);
            wp_enqueue_style('megatron_vertical_progress_bar_css', plugins_url('megatron-framework/includes/shortcodes/vertical-progress-bar/assets/css/vertical-progress-bar' . $min_suffix_css . '.css'), array(), false);
            $bar_options = array();
