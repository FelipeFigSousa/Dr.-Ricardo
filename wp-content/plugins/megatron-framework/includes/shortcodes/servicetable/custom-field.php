<div class="my_meta_control">
    ...
    <?php while($mb->have_fields_and_multi('features',array('length' => 2))): ?>
        ...
        <a onClick="buttonHandler(this)" class="button feature-button <?php if($mb->get_the_value()=="featured"){echo "g5plus-pt-featured";}else {echo "g5plus-pt-unfeatured";}?>" data-trigger="hover" data-html="true" data-placement="right" data-original-title="&lt;strong&gt;<?php esc_html_e('Feature This Column', 'g5plus-megatron'); ?>&lt;/strong&gt;" data-content="<?php esc_html_e("Click this button to feature this column. A featured column appears bigger and includes the wording 'Most Popular'. You can only feature one column per table.", 'g5plus-megatron'); ?>"><?php esc_html_e('Feature', 'g5plus-megatron'); ?></a>
        <a href="#" class="dodelete button">Remove</a>
        <?php $mb->the_field('planname'); ?>
        <label><?php esc_html_e('Name', 'g5plus-megatron'); ?></label>
        <input class="form-control" type="text" name="<?php esc_attr($mb->the_name()); ?>" value="<?php esc_attr($mb->the_value()); ?>"/>
        <?php $mb->the_field('planicon'); ?>
        <label><?php esc_html_e('Icon', 'g5plus-megatron'); ?></label>
        ...
        <span class="icon-preview"><i class=""></i></span>
        <?php $mb->the_field('plantext'); ?>
        <label><?php esc_html_e('Text', 'g5plus-megatron'); ?></label>
        <input class="form-control" type="text" name="<?php esc_attr($mb->the_name()); ?>" value="<?php esc_attr($mb->the_value()); ?>"/>
        <?php $mb->the_field('plansubtext'); ?>
        <label><?php esc_html_e('Sub Text', 'g5plus-megatron'); ?></label>
        <input class="form-control" type="text" name="<?php esc_attr($mb->the_name()); ?>" value="<?php esc_attr($mb->the_value()); ?>"/>

        <?php $mb->the_field('planfeatures'); ?>
        <label><?php esc_html_e('Features', 'g5plus-megatron'); ?></label>
        ...
        <label><?php esc_html_e('Button Text', 'g5plus-megatron'); ?></label>
        <input class="form-control" type="text" name="<?php esc_attr($mb->the_name()); ?>" value="<?php esc_attr($mb->the_value()); ?>"/>

        <?php $mb->the_field('buttonurl'); ?>
        <label><?php esc_html_e('Button Url', 'g5plus-megatron'); ?></label>
        <input class="form-control" type="text" name="<?php esc_attr($mb->the_name()); ?>" value="<?php esc_attr($mb->the_value()); ?>"/>
        <?php $mb->the_group_close(); ?>
    <?php endwhile; ?>
    <div style="clear: both;"></div>
    <p>
        <a href="#" class="docopy-features button"><?php esc_html_e('Add Features', 'g5plus-megatron'); ?></a>
    </p>
</div>
<style>
