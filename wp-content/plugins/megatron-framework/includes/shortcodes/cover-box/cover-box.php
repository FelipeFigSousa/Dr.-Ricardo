...
if (!class_exists('g5plusFramework_Shortcode_Cover_Box')) {
    class g5plusFramework_Shortcode_Cover_Box
    {
        ...
        {
            add_shortcode('megatron_cover_box_ctn', array($this, 'cover_box_ctn_shortcode'));
            add_shortcode('megatron_cover_box_sc', array($this, 'cover_box_sc_shortcode'));
        }
        function cover_box_ctn_shortcode($atts, $content)
        {
	        ...
	        $atts = vc_map_get_attributes( 'megatron_cover_box_ctn', $atts );
	        extract( $atts );
            ...
            wp_enqueue_script('megatron_cover_box_js', plugins_url('megatron-framework/includes/shortcodes/cover-box/assets/js/cover-box' . $min_suffix_js . '.js'), array(), false, true);
            wp_enqueue_style('megatron_cover_box_css', plugins_url('megatron-framework/includes/shortcodes/cover-box/assets/css/cover-box' . $min_suffix_css . '.css'), array(), false);
        ...
        function cover_box_sc_shortcode($atts,$content = nul)
        {
	         ...
	         */
            $image=$title=$link='';
	        $atts = vc_map_get_attributes( 'megatron_cover_box_sc', $atts );
	        extract( $atts );
            //parse link
            $link = ( $link == '||' ) ? '' : $link;
...
if ( class_exists( 'WPBakeryShortCodesContainer' ) ) {
    class WPBakeryShortCode_megatron_cover_box_ctn extends WPBakeryShortCodesContainer {
    }
}
if ( class_exists( 'WPBakeryShortCode' ) ) {
    class WPBakeryShortCode_megatron_cover_box_sc extends WPBakeryShortCode {
    }
}