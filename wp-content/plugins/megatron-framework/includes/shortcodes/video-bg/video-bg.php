...
if (!class_exists('g5plusFramework_Shortcode_Video_Bg')) {
	class g5plusFramework_Shortcode_Video_Bg
	{
		...
		{
			add_shortcode('megatron_video_bg', array($this, 'video_bg_shortcode'));
		}
		...
		function video_bg_shortcode($atts, $content)
		{
			 ...
			 * @var $delay
			 */
			$particles=$muted=$loop=$mp4_link =$ogg_link=$webm_link=$description = $image = $el_class = $g5plus_animation = $css_animation = $duration = $delay = '';
			$atts = vc_map_get_attributes( 'megatron_video_bg', $atts );
			extract( $atts );
			...
			wp_enqueue_style('megatron_video_bg_css', plugins_url('megatron-framework/includes/shortcodes/video-bg/assets/css/video-bg' . $min_suffix_css . '.css'), array(), false);
			...
			<div class="video-bg <?php echo esc_attr($g5plus_animation) ?>" <?php echo g5plusFramework_Shortcodes::g5plus_get_style_animation($duration, $delay); ?>>
				<?php if($particles=='yes'):
					$min_suffix_js = (isset($g5plus_options['enable_minifile_js']) && $g5plus_options['enable_minifile_js'] == 1) ? '.min' : '';
					wp_enqueue_script('megatron_video_bg_js', plugins_url('megatron-framework/includes/shortcodes/video-bg/assets/js/particles' . $min_suffix_js . '.js'), array(), false, true);
				?>
...
if ( class_exists( 'WPBakeryShortCodesContainer' ) ) {
	class WPBakeryShortCode_megatron_video_bg extends WPBakeryShortCodesContainer {
	}
}