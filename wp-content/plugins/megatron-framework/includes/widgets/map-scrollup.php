...
class G5Plus_Widget_Map_ScrollUp extends G5Plus_Widget
{
    public function __construct()
    {
        ...
        $this->widget_description = esc_html__("Display map and scroll up", 'g5plus-megatron');
        $this->widget_id = 'megatron-map-scroll-up';
        $this->widget_name = esc_html__('G5Plus: Map & Scroll Up', 'g5plus-megatron');
        $this->settings = array(
            'scroll_up_title' => array(
                'type' => 'text',
                'std' => 'Scroll up',
                'label' => esc_html__('Scroll up title', 'g5plus-megatron')
            ),
            'see_map_title' => array(
                'type' => 'text',
                'std' => 'See map',
                'label' => esc_html__('Link map title ', 'g5plus-megatron')
            ),
            'map_title' => array(
                'type' => 'text',
                'std' => 'Google map',
                'label' => esc_html__('Google map title ', 'g5plus-megatron')
            ),
            'map_layout'  => array(
                'type'    => 'select',
                'std'     => 'container',
                'label'   => esc_html__( 'Map layout', 'g5plus-megatron' ),
                'options' => array(
                    'container' => esc_html__('Container','g5plus-megatron'),
                    'full' => esc_html__('Full','g5plus-megatron')
                )
            ),
            'link_map' => array(
                'type' => 'text',
                'std' => '',
                'label' => esc_html__('Link to map', 'g5plus-megatron')
            ),
            'location_x' => array(
                'type' => 'text',
                'std' => '',
                'label' => esc_html__('Or embed google map location x ', 'g5plus-megatron')
            ),
            'location_y' => array(
                'type' => 'text',
                'std' => '',
                'label' => esc_html__('Google map location y ', 'g5plus-megatron')
            ),
            'zoom' => array(
                'type' => 'text',
                'std' => '12',
                'label' => esc_html__('Google map zoom ', 'g5plus-megatron')
            ),

            'height' => array(
                'type' => 'text',
                'std' => '',
                'label' => esc_html__('Google map height ', 'g5plus-megatron')
            )
            );
    ...
    function widget($args, $instance)
    {
        ...
        <div class="map-scroll-up">
            ...
            <?php if(!isset($link) || $link==''){?>
                <div class="megatron-map <?php echo esc_attr($map_layout) ?>">
                    <div class="row">
                        <div class="col-md-12">
                            <?php
                            $shortcode = sprintf('[megatron_google_map map_zoom="%s" location_x="%s" location_y="%s" marker_title="%s" map_height="%s" map_style="none"]', $zoom, $location_x, $location_y, $map_title, $height);
                            if(shortcode_exists('megatron_google_map')){
