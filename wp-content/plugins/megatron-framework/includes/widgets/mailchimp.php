...
class G5Plus_Widget_MailChimp extends g5plus_acf_widget
{
    public function __construct()
    {
        ...
        $this->widget_description = esc_html__("Display Mailchimp", 'g5plus-megatron');
        $this->widget_id = 'megatron-mailchimp';
        $this->widget_name = esc_html__('G5Plus: Mailchimp', 'g5plus-megatron');
        $this->settings = array(
            'id' => 'mailchimp_acf',
            'type' => 'rows',
            'title' => esc_html__('Mailchimp', 'g5plus-megatron'),
            'subtitle' => esc_html__('Display mailchimp on sidebar or in page.', 'g5plus-megatron'),
            'extra' => array(
                array(
                    'name' => 'title',
                ...
                array(
                    'name' => 'mailchimp_form',
                    'title' => esc_html__('Shortcode Mailchimp', 'g5plus-megatron'),
                    'type' => 'text',
                ...
                array(
                    'name'   => 'bg_overlay',
                    'title'   => esc_html__( 'Has background overlay',  'g5plus-megatron'),
                    'type'    => 'select',
                    'std'     => '1',
                    'allow_clear' => '1',
                    'options' => array(
                        '1'  => esc_html__( 'Yes', 'g5plus-megatron' ),
                        '0' => esc_html__( 'No',  'g5plus-megatron' )
                    )
                ),
            )
        );
    ...
    function widget($args, $instance)
    {
        ...
        $short_code_heading = sprintf('[megatron_heading title="%s" size="size-md" sub_title="%s" text_align="text-left" color_scheme="color-light"]', $title, $description);
